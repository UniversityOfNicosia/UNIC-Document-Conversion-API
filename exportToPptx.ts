...
export function exportToPptx(markdownString: string, fileName = 'presentation.pptx') {
    ...
    bodyElements.forEach((element, i) => {
        ...
        if (element.tagName === 'H1' || element.tagName === 'H2') {
            ...
            const lines = Math.ceil(textContent.length  / MAX_CHARS_PER_LINE); // 'textContent' is possibly 'null'.ts(18047)
            ...
            const lines = Math.ceil((blockquoteElement.textContent || '').length / MAX_CHARS_PER_LINE); // 'blockquoteElement.textContent' is possibly 'null'.ts(18047)
            ...
            const lines = Math.ceil((aElement.textContent || '').length / MAX_CHARS_PER_LINE); // 'aElement.textContent' is possibly 'null'.ts(18047)
            const increment = LINE_SPACING * lines;
            y += increment;
            slide.addText(aElement.textContent || '', { x: 0.5, y: y, fontSize: 14, hyperlink: aElement.href });
            y += increment;
        } else {
